...
def moveTowardsFlag( point, path, verbose=False ):
    ...
    _, pioneerPose = getPioneerPose()
    errorFlagPos, flagPos = readPositionSignal( "Flag" )
    pioneerPose = positionRelativeToPoint( "Flag", flagPos, errorFlagPos, pioneerPose, verbose )
    
    pose = pioneerPose.copy()
    pose = positionRelativeToPoint( "Point", path[point], False, pose, verbose )

    if point < len(path) - 1:
        nextPoint = pioneerPose.copy()
        nextPoint = positionRelativeToPoint( "Point", path[ point+1 ], False, nextPoint, verbose )
        if( nextPoint["DistanceToPoint"] < pose["DistanceToPoint"] ):
            point += 1
            pose = nextPoint
...
def braitenberg( vLeft, vRight, reactiveFlag, pose, reference, verbose=False ):
    """
    ...
    - reactiveFlag: whether last behavior was reactive or not
    ...
    if frontObstacle:
        ...
        if conversionFactor*np.min( dist[2:5+1] ) < pose["DistanceToPoint"] - uncertainty:
    ...
    return vLeft, vRight, reactiveFlag



def followWall( vLeft, vRight, reactiveFlag, pose, reference, verbose=False ):
    """
...
while sim.simxGetConnectionId(clientID) != -1:
    ...
    # vLeft, vRight, reactiveFlag = followWall( vLeft, vRight, reactiveFlag, pose, "Point", verbose=verbose )
    vLeft, vRight, reactiveFlag = braitenberg( vLeft, vRight, reactiveFlag, pose, "Point", verbose=verbose )
